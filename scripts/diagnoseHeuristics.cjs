...
console.log("=== EVALUATING JOURNEY LEVEL MATCHING ===");
for (const f of baseFares) {
  if (f.journey.isBus || f.journey.isCapHit || f.actualCharge <= 0 || f.expectedFare <= 0) {
    console.log(`Skipped (bus/cap/zero): "${f.journey.raw.journeyAction}" (Actual: £${f.actualCharge}, Expected: £${f.expectedFare})`);
    continue;
  }
  
  // Ignore penalty fares / maximum fares (e.g. £4.65, £8.90)
  if (f.actualCharge === 4.65 || f.actualCharge === 8.90) {
    console.log(`Skipped (penalty/max): "${f.journey.raw.journeyAction}" (Actual: £${f.actualCharge}, Expected: £${f.expectedFare})`);
    continue;
  }

  eligibleJourneysCount++;
  const diffToStandard = Math.abs(f.actualCharge - f.expectedFare);
  const diffToFareType = Math.abs(f.actualCharge - (f.fareTypeFare ?? f.expectedFare));

  let matched = 'none';
  if (diffToFareType < diffToStandard - 0.1) {
    fareTypeMatches++;
    matched = 'railcard';
  } else if (diffToStandard < diffToFareType - 0.1) {
    standardMatches++;
    matched = 'standard';
  }

  console.log(`Journey: "${f.journey.raw.journeyAction}"
  Actual: £${f.actualCharge.toFixed(2)} | Expected: £${f.expectedFare.toFixed(2)} | Railcard Expected: £${f.fareTypeFare.toFixed(2)}
  diffToStandard: ${diffToStandard.toFixed(2)} | diffToFareType: ${diffToFareType.toFixed(2)} | Matched: ${matched}`);
}

console.log(`\n=== RESULTS ===`);
console.log(`eligibleJourneysCount: ${eligibleJourneysCount}`);
console.log(`fareTypeMatches: ${fareTypeMatches}`);
console.log(`standardMatches: ${standardMatches}`);
console.log(`Ratio: ${fareTypeMatches / eligibleJourneysCount}`);
console.log(`Result: ${fareTypeMatches > standardMatches && (fareTypeMatches / eligibleJourneysCount) > 0.4}`);
